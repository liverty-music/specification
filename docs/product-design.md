...
> （アカウント紐付けチケット + サーバー署名の回転QR + Passkey 本人確認）で
> 実装する。**ブロックチェーン（SBT / ERC-4337 / ZKP 入場）は採用しない。**
>
> 理由: Liverty Music は単一運営のウォールドガーデン（Scenario A）であり、
> ブロックチェーン固有の性質が load-bearing になる「信頼境界を跨ぐ状況」
> に該当しない。チケット不正転売禁止法への対応も本人確認 + 非譲渡指定で
> off-chain で達成できる。かつて存在した SBT/ZKP 実装は
> `remove-blockchain-ticket-system` で撤去済み。
>
> **再検討する唯一の条件**: 複数プロモーターによるオープンな二次流通市場、
> またはクロスプラットフォーム相互運用が現実の要件になった場合のみ。
>
> 根拠の一次情報（重複管理を避けるため、詳細はここに複製しない）:
> `openspec/changes/archive/2026-08-09-remove-blockchain-ticket-system/`
> の proposal.md / design.md、および issue liverty-music/specification#741。
