...
Defines what an unauthenticated (guest) user may access: which routes are reachable (free roam after the dashboard), the principle that account-only features are hidden rather than navigation-blocked, and the persistent sign-in / sign-up entry point available from the discovery step onward.
