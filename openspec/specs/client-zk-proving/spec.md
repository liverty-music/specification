# client-zk-proving Specification

## Purpose

The browser-side zero-knowledge proving runtime that lets a fan prove event-entry eligibility on-device. It defines the runtime's license constraint (no copyleft code in the distributed bundle), its output contract (a BN254 Groth16 proof the existing `gnark` / `vocdoni/circom2gnark` backend verifies), on-device privacy, offline capability, and circuit-artifact integrity verification.
