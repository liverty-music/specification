# venue-name-localization Specification

## Purpose

The Venue Name Localization capability selects the appropriate venue display name based on the user's preferred language. Japanese users see the scraped `listed_venue_name` (typically in Japanese, from the artist's official site), while English users see the canonical `venue.name` from Google Places. Each direction falls back to the other field when the preferred field is absent.
