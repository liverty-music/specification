# prod-image-tag-immutability Specification

## Purpose

Defines the contract that container image references in production are both **readable** (semantic version tags like `v1.0.0` in kustomize manifests) and **immutable** (registry-level enforcement via GCP Artifact Registry's `dockerConfig.immutableTags: true`). Combined, these give at-a-glance version visibility in manifests + GitOps reproducibility equivalent to digest pinning at the AR boundary. Scope is intentionally narrow to **non-dev** environments (the Pulumi conditional is fail-secure `environment !== 'dev'`); dev AR repositories stay mutable so ArgoCD Image Updater can rewrite `:latest` / `:main` tags during dev iteration. The Kubernetes Recommended Label `app.kubernetes.io/version` propagates the release version onto every prod Deployment + CronJob (and their Pod templates) so Prometheus / OTel observability sees the version dimension. Strictly stricter than `prod-image-pipeline`: this capability forbids commit-SHA-only `newTag:` values that the broader pipeline spec permits.

## Requirements
### Requirement: Non-dev Artifact Registry Docker repositories SHALL enforce tag immutability at the registry API level

Every Docker-format `gcp.artifactregistry.Repository` resource provisioned in any **non-dev** liverty-music GCP project (currently `liverty-music-prod`; in the future also any staging / prod-replica project that gets provisioned) SHALL declare `dockerConfig.immutableTags = true`. The Pulumi conditional SHALL be expressed in the fail-secure negative form (`environment !== 'dev'`) so that any future non-dev environment automatically inherits the policy without an opt-in code change — see design.md D2 for the full rationale. The Artifact Registry API SHALL reject any attempt to re-point a tag **that was written after `immutableTags` was enabled on the repository** to a different image digest (whether issued via `docker push`, `gcloud artifacts docker tags add`, or the AR REST API), returning HTTP 409 Conflict. Dev-project AR repositories SHALL remain at the default (`immutableTags = false`) because ArgoCD Image Updater requires the ability to rewrite `:latest` and `:main` tags during dev iteration.

Today only `liverty-music-dev` and `liverty-music-prod` projects exist; the prod-only scenarios below are the only operationally enforceable ones. Future staging projects, when provisioned, SHALL be added to the scenario list (matching their AR repos against `immutableTags == True`) by a follow-up change.
...
**Relationship to `prod-image-pipeline` (cross-spec note):** The in-flight `prod-image-pipeline` capability (added by the active `prepare-prod-service-in` change, archive pending) contains a scenario that permits `:vX.Y.Z` OR `:<sha>` for prod overlay image URIs ("Image tags are explicit, never `:latest`"). This new `prod-image-tag-immutability` capability is **strictly stricter**: it requires semver only and forbids commit-SHA-only `newTag:` values. When both specs land in main (after `prepare-prod-service-in` is archived), the stricter `prod-image-tag-immutability` rule SHALL govern — it eliminates the `:<sha>` allowance that the broader `prod-image-pipeline` scenario permitted. This is intentional: the broader allowance existed because at the time `prepare-prod-service-in` was authored, AR Immutable Tags was not yet enabled and SHA pinning was the only practical reproducibility mechanism. With this change, semver + immutable-tags supersedes SHA-only pinning as the prod convention.
