# proximity-model Specification

## Purpose

The `proximity-model` capability defines the domain model for geographic proximity classification between a user's home area and concert venues. It provides the `Proximity` enum, the `Concert.ProximityTo` entity method, and the `pkg/geo.Haversine` utility function.
