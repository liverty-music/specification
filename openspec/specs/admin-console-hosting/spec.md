...
Serving and delivery for `admin.liverty-music.app`: a separate container image and Kubernetes Deployment/Service, a dedicated HTTPRoute hostname on the shared external gateway, certmap + Cloud DNS entries, per-host runtime config delivery, the ArgoCD/image-updater wiring, and the admin-org `ApplicationOidc` Zitadel resource — all isolated from the consumer SPA's hosting and release cadence.
