...
- [ ] 7.2 End-to-end verify: configure phase → apply (authorize, no capture) → close → draw (no oversell, groups intact, ordered loser waitlist; **winners captured / losers released**) → ⑤ issuance handoff — logic already covered (backend `PaymentAuthorizationPort` mocks + frontend stripe-service mocks, CI green); only the real Stripe authorize→3DS→capture/cancel round-trip is unverified. This needs **Stripe test-mode keys only** (`sk_test_`/`pk_test_`; free, no KYC) — **NOT the Connect KYC/審査 of 0.1** (0.1 is for the live sale; the implemented adapter is a plain manual-capture PaymentIntent with no Connect `on_behalf_of`/`application_fee`, so no connected account is needed). Runnable locally (docker-compose backend + npm frontend), independent of the intentionally-stopped dev env.
