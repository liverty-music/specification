...
- [ ] 8.3 Ensure prod runs the lottery-capable build — **frontend DONE** (prod pin **v1.63.0** contains fe #577 lottery UI, verified). **Backend PENDING**: prod fan-api is **v1.51.0**, which PREDATES lottery backend #426 (merged 2026-09-02) — so the lottery RPCs + Atlas migration `20260902010000_add_lottery_tables` are NOT in prod yet. Needs a **backend Release (≥ v1.52.0 off current main, includes #426)** → prod AR + auto prod-pin + Atlas operator applies the lottery tables. (No Connect params / no live KYC for test-mode.)
