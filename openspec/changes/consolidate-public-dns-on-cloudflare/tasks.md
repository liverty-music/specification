...
- [ ] 9.3 **Prod preview expectation**: ~9 creates (3 Cloudflare A records for apex/api/auth; 3 ACME CNAMEs; 3 new resources for the apex Cert chain: `web-app-cert` + `web-app-dns-auth` + `web-app-cert-map-entry`; the api/auth Cert chain is unchanged in identity — only the ACME CNAME provider flips) + ~11 destroys (2 ManagedZones `api-liverty-music-app-public-zone`, `auth-liverty-music-app-public-zone` + 6 NS-delegation Cloudflare records [3-4 each for api + auth] + 3 old api/auth `gcp.dns.RecordSet` inside the zones) + ~1 update on `api-gateway-cert-map` (gains apex entry). Note: exact NS-record count depends on observed GCP nameserver count (3 or 4); adjust expectation to match preview.
