...
- [ ] 1.7 Confirm the prod frontend HTTPRoute already binds the apex hostname (pre-existing from `prod-k8s-manifests` 2026-05-14). Run `grep -rn "liverty-music.app" cloud-provisioning/k8s/namespaces/frontend/overlays/prod/`; expected output contains a strategic-merge patch line setting the apex hostname (the patch value sets `/spec/hostnames` on the HTTPRoute, currently inlined in `kustomization.yaml` as `value: ["liverty-music.app"]`). If no such occurrence is found anywhere under the prod overlay, **HALT** — fix it in a follow-up PR to `cloud-provisioning` before this change's prod cutover. (Satisfies the apex-frontend-serving capability's "Frontend HTTPRoute binds apex hostname" scenario without authoring HTTPRoute YAML in this change.)
...
- [ ] 9.3 **Prod preview expectation**: ~7 creates (3 Cloudflare A records for apex/api/auth; 3 Cloudflare ACME CNAMEs; 1 new apex `web-app-dns-auth` DnsAuthorization — note: `web-app-cert` and `web-app-cert-map-entry` may show as create or as update depending on Pulumi state — verify against actual preview) + ~12 destroys (2 ManagedZones `api-liverty-music-app-public-zone`, `auth-liverty-music-app-public-zone` + 6 NS-delegation Cloudflare records [3-4 each for api + auth] + 4 old `gcp.dns.RecordSet` inside the zones [2 A + 2 ACME CNAME for api/auth]) + ~1 update on `api-gateway-cert-map` (gains apex entry). The api/auth Cert chains are unchanged in identity — only their backing ACME CNAME resource type flips. Note: exact NS-record count depends on observed GCP nameserver count (3 or 4); adjust expectation to match preview.
