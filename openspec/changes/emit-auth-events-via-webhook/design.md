...
### Decision 2: Verify the Target with `PAYLOAD_TYPE_JSON` + HMAC (NOT `PAYLOAD_TYPE_JWT`)

**Initial approach (superseded):** the Target was first provisioned as `PAYLOAD_TYPE_JWT`, reusing the `pre_access_token` handler's JWKS validator. **This failed in prod** — the event execution fired correctly but every target call errored with `Errors.WebKey.NoActive` (`internal/execution/execution.go:63`) and the webhook was never delivered. Root cause: an Actions v2 **event**-execution JWT target signs its payload with an **active instance Web Key**, and this Zitadel v4.14.0 instance has none (the `web_key` feature was never enabled; cloud-provisioning does not manage web keys). The `pre_access_token` **function** execution is unaffected because it signs in the OIDC token-issuance context (legacy signing key), not via the web-key path. Activating a web key was rejected: it would switch instance-wide OIDC signing to web keys — a sign-in-risking change on a fragile prod instance.

**Chosen:** `payloadType: 'PAYLOAD_TYPE_JSON'` with HMAC, which is [Zitadel's documented default and recommended verification](https://zitadel.com/docs/guides/integrate/actions/testing-request-signature) and does **not** require a web key. Zitadel generates a `signingKey` at Target creation and signs each body with a `ZITADEL-Signature` header. The backend handler verifies it with the Zitadel Go SDK: `actions.ValidateRequestPayload(body, &r.Header, signingKey)` (`github.com/zitadel/zitadel-go/v3/pkg/actions`, already a dependency). The body is the raw event JSON (`{event_type, event_payload, userID, …}`) — no JWT wrapping — so the handler unmarshals it directly; `event_payload` is still base64-decoded to read the login `userID`. `interruptOnError: false` (analytics must never block login).

**Signing-key plumbing.** Zitadel owns the generated `signingKey`, so it is captured from the Target (Pulumi output) → stored as GSM secret `webhook-login-event-signing-key` → synced into `backend-secrets` via ExternalSecret as `WEBHOOK_LOGIN_EVENT_SIGNING_KEY`. Because ESO v1beta1 fails the whole `backend-secrets` bundle when a referenced remote key is absent, the ExternalSecret entry is added in a **follow-up step only after** the prod `pulumi up` has created the GSM secret (the same two-phase pattern `posthog-public-project-key` used). Until the key is synced, the handler rejects unverifiable requests (401) — analytics degrades, login is unaffected. The webhook listener remains a private, internal-only port as defence-in-depth.
