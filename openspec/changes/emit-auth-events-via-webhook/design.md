...
The login signal is a Zitadel Actions v2 **`event` Execution** (condition `event.event = "<login-event-type>"`) invoking a webhook **Target** that points at the backend handler. An `event` execution fires **after** the login event is persisted to the Zitadel eventstore, receives the stored event payload (`{aggregateID, event_type, userID, event_payload}`), and **cannot manipulate any API request or response**. This is Zitadel's documented pattern for reacting to events / running side-effects ([Actions v2 concepts](https://zitadel.com/docs/concepts/features/actions_v2), [Actions v2 usage](https://zitadel.com/docs/guides/integrate/actions/usage)).
...
### Decision 1b: Determine the login `event_type` empirically via the Events API — do NOT guess

Zitadel event type names are not guessed. Before configuring the Execution, discover the correct `event_type` with the **Events API** (`ListEvents`, `POST /admin/v1/events/_search`): perform a fresh interactive login and a silent `refresh_token` grant, list the events each produced, and select the type that (a) fires **once** per user-initiated login, (b) does **not** appear on the refresh, and (c) carries the logging-in user's Zitadel `userID` on its aggregate. The recorded `ListEvents` output (the two event lists and the chosen type) is the build-time justification for the Execution's `event.event` value. This replaces the old gating "payload spike" — that spike only existed to evaluate the discarded `pre_access_token` hook; the equivalent empirical step here is naming the login event, not discriminating a grant at runtime.
...
`Enqueue` needs the platform `UserId`; the `event`-execution payload carries the Zitadel `userID` (the login event's user aggregate). The handler resolves `userID → UserId` via the existing `UserUseCase.GetByExternalID` (already defined in `user_uc.go`, "retrieves a user by identity provider ID (Zitadel sub claim)"). The event fires only on a user-initiated login, never on refresh, so refreshes incur no extra query. The resolved `UserId` becomes the NATS event payload's `user_id` and ultimately the PostHog `distinct_id`. If the lookup fails (user not yet provisioned, or transient error) or the payload `userID` is absent, the handler logs and skips the analytics emission — and because the execution is fire-and-forget, the handler's response never affects login regardless.
