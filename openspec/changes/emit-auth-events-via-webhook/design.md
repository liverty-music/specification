...
`Enqueue` needs the platform `UserId`; the `session.user.checked` payload carries the Zitadel user at `payload.userID` (the `editor` is the Login-UI service user, not the end user, so `payload.userID` is the correct field). The handler resolves `userID → UserId` via the existing `UserUseCase.GetByExternalID` (already defined in `user_uc.go`, "retrieves a user by identity provider ID (Zitadel sub claim)"). The event fires only on a user-initiated login, never on refresh, so refreshes incur no extra query; and because a machine `jwt_profile` grant never produces `session.user.checked`, no M2M traffic reaches this handler at all. The resolved `UserId` becomes the NATS event payload's `user_id` and ultimately the PostHog `distinct_id`. If the lookup fails (user not yet provisioned, or transient error) or `payload.userID` is absent, the handler logs and skips the analytics emission — and because the execution is fire-and-forget, the handler's response never affects login regardless.
