...
- [ ] 5.2 Merge spec deltas into canonical `openspec/specs/prod-image-pipeline/spec.md`:
  ...
  - **REMOVED**: The "Image tags are explicit, never `:latest`" scenario under "Prod kustomize overlays SHALL pin image URIs to prod-AR paths" — superseded by `prod-image-tag-immutability`'s semver-only rule.
