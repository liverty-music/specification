...
- [x] 10.1 After BSR release, run `npm install @buf/liverty-music_schema.connectrpc_es@<new-version>` to consume the new generated types. Pinned `@buf/liverty-music_schema.bufbuild_es@1.10.0-20260524050818-e7f694dd726b.1` and `@buf/liverty-music_schema.connectrpc_es@1.6.1-20260524050818-e7f694dd726b.2` in frontend@a11f30f. The `.2` suffix on the connectrpc_es build is BSR's per-commit build counter (a second codegen run for the same proto commit; harmless, just means BSR ran the plugin twice and the second result is canonical). Pinned the v1.x lineage for both because no v2.x build of connectrpc_es is published yet for this BSR commit — bufbuild_es ships both v1 and v2 lineages automatically for base types, but the Connect-ES generator only has a v1 plugin published in the BSR registry, so v1 is the only coherent pair today.
...
- [x] 11.2 After merge, create a GitHub Release with a SemVer-major tag (since this is a breaking change). The `buf-release.yml` workflow will publish to BSR. **Convention override**: tagged **v0.41.0** (minor bump) rather than v1.0.0 because the team treats the 0.x lineage as breaking-allowed and has used minor bumps for every prior breaking proto change (v0.36 → v0.40 history). The "SemVer-major" wording in this task's original ask reflects strict SemVer; we knowingly deviate. Future post-1.0 changes will use true major bumps. Release notes summarise the schema changes and migration notes.
- [x] 11.3 Monitor `gh run watch` on the BSR release workflow until completion. `Buf BSR Push on Release` workflow run id 26352579947 started at 2026-05-24T05:08:12Z and completed SUCCESS at 2026-05-24T05:08:22Z (a 10-second push). The published BSR build is timestamped `20260524050818` (mid-run, when the plugin produced the artifact). Go package publication was verified by the successful `go get` in 9.1 against `buf.build/gen/go/liverty-music/schema/protocolbuffers/go` and `.../connectrpc/go`; npm package publication was verified by `npm view @buf/liverty-music_schema.bufbuild_es versions --json` and the equivalent for `connectrpc_es`, both of which return the `e7f694dd726b` build.
