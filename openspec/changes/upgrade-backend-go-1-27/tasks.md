...
- [ ] 5.2 Validate the metric ingests in GMP and the alert previews without apply-time errors (disableMetricValidation if not-yet-ingested) — _TypeScript compiles (biome + tsc green); the alert uses `conditionThreshold` on `workload.googleapis.com/backend_goroutine_leak_count` (OTLP→googlecloud exporter path, not GMP/PromQL — so `disableMetricValidation` is N/A). Live `pulumi preview` + GMP ingestion confirmation are deploy-gated (need cloud credentials)._
