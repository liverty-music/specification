...
- [ ] 6.5 Anonymous capture posture: capture the **full non-PII catalogue** before/without identification using `localStorage` persistence + anonymous id (so anonymous funnels survive reload); remove the closed pre-consent allowlist; no account-mapped identifier on anonymous events
