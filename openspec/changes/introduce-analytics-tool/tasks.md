...
- [ ] 3.1 Create the `analytics-consumer` package under `backend/internal/adapter/event/analytics_consumer/` with subscription configuration for the relevant NATS subjects (`user.*`, `ticket.purchase.*`, `ticket.lottery.*`, `entry.zk_proof.*`, `push.notification.delivered`, `artist.follow.*`)
