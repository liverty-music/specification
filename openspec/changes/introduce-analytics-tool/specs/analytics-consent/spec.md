...
#### Scenario: Analytics toggle defaults to on; marketing toggle defaults to off
- **WHEN** the consent screen is rendered for the first time
- **THEN** the analytics consent toggle SHALL default to on (opt-out)
- **AND** the marketing-measurement consent toggle SHALL default to off (opt-in)
