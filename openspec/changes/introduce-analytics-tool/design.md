...
Identified analytics is **enabled by default** for authenticated users; the user can opt out at any time from a settings control. There is no signup consent gate. The final onboarding step is a one-time, non-blocking **transparency notice** naming PostHog (Netherlands) and the cross-border purpose, linking to the privacy policy and the settings opt-out. Before identification — anonymous visitors and authenticated-but-opted-out users — the app captures the **full non-PII catalogue** anonymously (anonymous visitors MAY use `localStorage` so anonymous funnels survive reloads; opted-out authenticated users stay memory-only), with IP configurable and no account-mapped identifier. On login the anonymous profile is **merged** into the identified profile. The settings section carries two opt-out toggles — **Analytics** and **Session replay** (the latter is the former `marketingMeasurement` field, renamed) — both defaulting on. 要配慮個人情報 and minor-identifying data are excluded structurally at the property/replay layer.
...
- **[Risk] Opt-out model draws privacy complaints or a higher-than-expected opt-out rate → Mitigation**: per Decision 7 analytics is on by default (cleared by EU adequacy) but the opt-out is always available in settings and surfaced by a non-blocking onboarding transparency notice; 要配慮 and minor data are excluded structurally. Measure post-launch the opt-out rate and complaint volume; if the opt-out rate is high, improve the transparency copy and the perceived value exchange rather than reverting to opt-in.
