...
### Decision 11: Event taxonomy stays hand-maintained now; schema-generation is the explicit scale-up path

The taxonomy currently exists as **three hand-maintained copies of one truth**: the catalogue (`docs/analytics/event-catalog.md`), the backend constants (`backend/internal/usecase/analytics_events.go`), and the frontend constants (`frontend/src/services/analytics-events.ts`). Tasks 7.4 and 9.2 originally proposed CI guards to *police* divergence between them.

The industry norm for tracking-plan management is the opposite paradigm — **define the taxonomy once and generate the typed clients (and docs) from it**, so the copies are outputs of a single source and cannot diverge by construction. This is what Avo Codegen, Amplitude Ampli, and Segment Protocols do, and the in-house variant (define a schema, codegen Go + TS) is well documented. "Validate parallel copies" (our original plan) only *detects* drift; "generate from one source" *eliminates the drift class*.

This repo already runs the generate-from-one-source pattern for its API: `buf.gen.yaml` produces synchronized Go **and** TypeScript from a single proto schema via BSR. The event taxonomy is structurally the same problem.

**Decision:** Do **not** build the cross-repo event-catalogue drift guard (task 9.2). While the taxonomy is small and effectively single-author, rely on the controls already in this design — PR review against the catalogue (Decision 5 mitigation) and **type-level** default/name enforcement (the backend `FeatureFlagEvaluator` makes defaults a required parameter; the typed `Events`/`AnalyticsProperties` maps make event names/props compile-time-checked). This is proportionate to a low-severity constraint (catalogue staleness degrades documentation, it does not break production).

**Adopt schema-generation (Paradigm B) when any trigger fires:** the taxonomy outgrows ~one screen of events, a second regular contributor starts adding events, or a paired-event drift bug actually reaches `main`.

**Paradigm B shape (recorded so the future move is cheap):** one proto `message` per event under `proto/liverty_music/analytics/v1/events.proto`, properties as protovalidate-constrained fields, plus a message-option annotation carrying the dot.case `name`/`source`/`paired_with` that cannot live in a proto identifier. Property structs (Go) + interfaces (TS) + runtime validation come free from the existing `protocolbuffers/go` / `bufbuild/es` / `validate-go` plugins; the catalogue markdown can be generated by `protoc-gen-doc`. The only bespoke piece is a small descriptor-walking generator that emits the dot.case name constants from the annotations. Operational cost: every taxonomy change becomes a BSR release cycle (proto PR → Release → BSR gen → `go get`/`npm install`) rather than a one-line constant edit — acceptable only once drift actually bites.

**Alternatives considered:**

- *Cross-repo CI drift guard (original task 9.2)*: rejected as the standing plan. It pays a permanent cost to police a structure we chose to make drift-prone, and faces a locus trilemma — the check must run where a constant is added (a code repo) but the truth lives in another (specification), forcing a choice between synchronization cost (vendored copy), timing cost (specification-side detection that cannot gate the introducing PR), or coupling cost (a cross-repo read credential in every code repo). All three manage drift rather than removing it.
- *Adopt a SaaS tracking-plan tool (Avo / Amplitude Ampli / Segment Protocols)*: the heavyweight industry answer; rejected for now because it adds a SaaS dependency, cost, and another vendor in the consent surface, none justified at the current taxonomy size.
...
### Decision 13: Operational/pipeline telemetry stays in OpenTelemetry, not PostHog (PostHog↔OTel boundary, applied)

Decision 10 draws the line: PostHog receives product/user events in domain vocabulary; OpenTelemetry receives system telemetry. Two catalogued events fail that test and are reclassified out of the PostHog scope:

- **`concert.search.completed` (task 14.5)** — emitted by the background concert-discovery worker (CronJob / follow-triggered goroutine), per-artist, with **no user**. Its consumers are "Gemini discovery success rate, search-pipeline health" — i.e. system health, not user behaviour. The `Enqueue` contract requiring a UUID `distinct_id` (Decision 3) is the structural signal: an event with no user is not product analytics. **Resolution: descope from PostHog; the search-pipeline success/health signal belongs in the backend's OTel/Prometheus metrics** (alongside the existing `analytics_consumer_*` series). No PostHog publisher is implemented for it.
- **`concert.recommendation.served` (task 13.5)** — the catalogue presumes a versioned ranking engine (`algorithm_version`, `position`). Verified: **no recommendation engine exists and none is planned** (not in `product-roadmap.md`, not in any active change); the product serves a concert *feed* via `List`/`ListByFollower`/`ListWithProximity` queries. The event is speculative catalogue content with no backing feature. **Resolution: descope.** It re-enters scope only if/when a recommendation feature is actually proposed; until then carrying a `[ ]` task + catalogue row is phantom debt. (The shipped FE `concert.recommendation.clicked` is a loose label over feed-card taps and is left as-is.)

**Rationale:** "What tool does this signal belong in?" is answered by "does it describe a user or the system?" — not by "can we make it fit." Forcing a no-user pipeline metric through the user-keyed analytics contract, or instrumenting a recommendation algorithm that doesn't exist, both violate the design's own boundaries.

### Decision 14: Notification-lifecycle analytics requires a notification entity; deferred until a product consumer needs it

`notification.delivered` (task 13.2) and `notification.opened`/`dismissed` (task 5.7) describe a notification's lifecycle (created → delivered → opened → dismissed). But push notifications are **fire-and-forget** — the `2026-02-22-push-notification-new-concerts` change deliberately dropped the `notification.go` entity for a minimal `PushSubscription` and recorded that a notification-log table is "required for post-MVP features (delivery tracking, retry, in-app inbox)". There is therefore **no `notification_id`** and no per-notification delivery/read state to report.

The industry-standard maturation is to make a notification a **first-class persisted entity** (a notification log + an outbox/dispatcher that fans out to channels and records per-channel delivery/read state). That single capability unlocks `notification_id`, the full delivered/opened/dismissed lifecycle (13.2, 5.7), an in-app inbox, and delivery audit/retry (directly relevant given the prod incident where sales-reminder pushes silently delivered nothing for weeks).
