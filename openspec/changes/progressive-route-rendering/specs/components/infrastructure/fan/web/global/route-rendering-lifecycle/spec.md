...
Governs how a data-fetching route paints something meaningful immediately on navigation, defers expensive or off-screen content until after that first paint, and restores a fan's scroll position when they return, so navigation never blocks on data.
