...
- Database hosting: reuse the existing Cloud SQL `postgres-osaka` (PG18) with a dedicated `zitadel` database and `zitadel-sa@...iam` IAM-authenticated role. Cloud SQL Auth Proxy sidecar with `--auto-iam-authn` removes the need for a password-bearing DSN secret.
