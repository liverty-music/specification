...
- **Lint script: keep `entity.hype` namespace as an empty placeholder or remove entirely?** → **Remove entirely.** Both the `entity.hype.label` key and the `entity.hype.values.*` subtree are deleted from JA and EN `translation.json`. The brand-vocabulary lint script's curated entity-stem list (`scripts/known-entities.ts`) no longer includes `hype`; any re-introduction of an `entity.hype.*` key will fail the unknown-stem check and act as the violation trigger described in the brand-vocabulary spec delta.

- **Should the my-artists `loading()` set `showSignupBanner` synchronously before the artist list resolves, or after?** → **After (post-resolve).** Keep the assignment inside the `finally` block of `loading()`, so the banner appears together with the artist list rather than flashing in during the loading-spinner state. The signup-prompt-banner spec scenario "Banner appears for guest user during onboarding" states "the banner SHALL be present from the moment the artist list has finished loading" — this codifies the post-resolve guarantee rather than the pre-resolve timing.
