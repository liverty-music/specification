...
- [x] 1.3 Add `DATABASE_SCHEMA=liverty_music` to `k8s/namespaces/backend/base/server/configmap.env`
- [x] 1.4 Run `kubectl kustomize` dry-run to verify K8s manifests

## 2. Backend Config (backend)

- [x] 2.1 Add `Schema` field to `DatabaseConfig` in `pkg/config/config.go` with `DATABASE_SCHEMA` env var and `public` default
- [x] 2.2 Update `GetDSN()` to include `search_path` parameter
- [x] 2.3 ~~Update `atlas.hcl` dev database URL~~ — Not needed: atlas dev URL uses an ephemeral Docker container for diffing; `public` is correct there
