...
Every Connect-RPC call issued by the frontend SHALL be bounded by a default client-side deadline so that a hung or unresponsive backend fails within a known window instead of leaving the request pending indefinitely. The default deadline SHALL be applied uniformly to all frontend applications (fan-web, admin, organizer) through the shared transport, and SHALL NOT require per-call or per-method configuration at the call site.
...
#### Scenario: Configuration supplies a timeout value

- **WHEN** `/config.json` provides an RPC timeout value at bootstrap
...
- **AND** renewal SHALL be retried on a later `pushsubscriptionchange` event or on next app open
