...
- [ ] 1.4 Define `SalesPhase` message: `id`, `series_id` (required), `method`, `channel`, `provider_name`, `sequence`, nullable `apply_start_time`/`apply_end_time`/`lottery_result_time`/`payment_deadline_time`, nullable `url` (reuse `Url` VO); use `_time` naming, no `_at`
...
- [ ] 2.3 Create `sales_phase_reminders` sent-log table with unique `(user_id, sales_phase_id, stage)` (reference the phase surrogate id, not series_id+sequence)
- [ ] 2.4 Apply migration locally and verify with `atlas migrate apply --env local`; confirm `ticket_emails` is unchanged
...
- [ ] 3.2 Define `SalesPhaseRepository` interface (Upsert by logical key, ListUpcomingByDueWindow, GetBySeries)
- [ ] 3.3 Implement pgx-based `SalesPhaseRepository` with `(series_id, channel, sequence)` upsert and last-write-wins on timeline/url/provider_name
...
- [ ] 4.5 Write unit tests with mock Gemini responses (verbatim → coerce → SalesPhase shaping; empty grounding → no phase)
...
- [ ] 5.4 Write use-case tests (idempotent re-run produces no duplicates)
- [ ] 5.5 Add `cronjob/sales-phase-discovery/cronjob.yaml` (daily) in cloud-provisioning

## 6. Multi-Stage Reminders (backend + cloud-provisioning)

- [ ] 6.1 Implement `internal/usecase/sales_reminder_uc.go`: scan due phases, compute stages (open / 24h-before / 1h-before / result-day), resolve series→performers→followers reusing the hype filter from `NotifyNewConcerts`
