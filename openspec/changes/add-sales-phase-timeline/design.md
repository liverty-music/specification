...
Each phase has a surrogate `id` (UUID PK) as its only hard identity and the handle the reminder sent-log references. The discovery job re-runs over the same series, so re-extraction must converge to the same row without collapsing two distinct phases. No business field is perfectly stable: `channel`/`sequence` may default (`UNSPECIFIED`, `0`) for unclassifiable phases, `apply_start_time` is revised as announcements firm up, and even "earliest covered event" drifts because coverage is resolved incrementally (a later run can add an earlier date). So matching is **best-effort by `(series_id, channel, sequence)` plus covered-event overlap**: a freshly extracted phase resolves to an existing row when it shares `(series_id, channel, sequence)` and its covered events overlap that row's. A frozen `anchor_event_id` (earliest covered event at first insert) is stored as a representative but is **never recomputed for matching**, so incremental coverage growth still overlaps the existing row instead of re-keying into a duplicate. Per-leg phases (first/second half) have disjoint coverage → no overlap → separate rows. All other fields (`apply_start_time` and the remaining timestamps, `provider_name`, `url`, covered-event set) are last-write-wins. Residual ambiguity (genuinely distinct same-`(series, channel, sequence)` phases with overlapping coverage) is rare and accepted as a known limitation — at worst a redundant row.
