...
The "re-run failed job is safe" claim relies on `crane copy` to an immutable tag at the same digest being a no-op — this MUST be regression-tested in the integration step (tasks §3.x).
