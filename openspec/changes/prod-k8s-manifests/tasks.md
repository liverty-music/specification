...
- [ ] 9.4 Watch ArgoCD reconcile (per design D6 — dev's actual 3-wave pattern):
  ...
  - Wave **0** (default — no annotation): most Apps in parallel, including `argocd` (self-management), `external-secrets`, `reloader`, `keda`, `nats`, `atlas-operator`, `otel-collector`, `gateway`, `backend-migrations`, `backend`, `frontend`, `zitadel`. ArgoCD's intra-wave dependency resolution handles ordering (controllers come up before workloads that depend on their CRDs/Services). When the first Zitadel API Pod boots, the `bootstrap-uploader` sidecar populates `zitadel-machine-key-for-pulumi-admin` with the generated admin JWT-profile key. Expected state at this point: Zitadel reaches Healthy; backend Pods stay in `ContainerCreating` waiting for `zitadel-machine-key-for-backend-app` (which doesn't exist yet — created in §9.7).
...
- [ ] 9.7 Trigger a **second** `pulumi up --stack prod` from Pulumi Cloud console. Now that `zitadel-machine-key-for-pulumi-admin` has a version (from §9.6), Pulumi can authenticate to the prod Zitadel API as org-admin. The Pulumi `MachineKey` resource for the backend creates the backend's lower-privilege machine user + key inside Zitadel, and Pulumi writes the JWT-profile to a **separate** GSM Secret `zitadel-machine-key-for-backend-app` (per the canonical `zitadel-self-hosted-deployment` spec lines 164 + 175). The org-admin JWT itself is NEVER mounted into any Pod.
- [ ] 9.8 Verify the backend machine-key Secret now exists: `gcloud secrets versions list zitadel-machine-key-for-backend-app --project liverty-music-prod` returns ≥1 enabled version.
- [ ] 9.9 Watch ArgoCD reconcile the backend Application — ESO now syncs `zitadel-machine-key-for-backend-app` into the backend namespace; reloader picks up the new K8s Secret and rolls the backend Deployment. Backend Pods leave `ContainerCreating` and reach Running. The backend → Zitadel auth path is now live.
