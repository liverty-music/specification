...
- **Persistence is per-series, and cheap to re-run** — the `series_id` is resolved ONCE per discovered series group via a cheap `(artist, dates)` DB lookup (adopt the `series_id` of any event this artist already performs on one of the dates, else mint one fresh `UUIDv7`) — needing NO Google Places API. The `series` row is created at that point, so every publishable event is inserted under an already-existing shared `series_id`. Google Places venue resolution is **deferred to only genuinely-new dates**, so re-discovering a known single-artist tour costs zero Places API calls (today it makes one per event, every run).
