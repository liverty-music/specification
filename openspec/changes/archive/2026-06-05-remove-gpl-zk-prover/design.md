...
- **`ark-circom` browser path is less battle-tested than its mobile-native path** (mopro's core is iOS/Android UniFFI bindings; Liverty Music is PWA-first and needs the pure-browser WASM path) → Mitigation: verification gate 2 spikes the actual browser build and benchmarks on real mobile browsers before adoption.
...
## Open Questions — RESOLVED by verification gates (2026-06-05)

- **A vs B final decision** → **Path A (replace).** Both gates passed (see `tasks.md` Gate 1 & Gate 2 Findings). The GPL code is eliminated physically.
- **R1CS identical?** → **YES, identical → reuse.** A structurally-identical Poseidon yields a byte-identical R1CS (`sha256 a8a5a29…`); the committed `.zkey` (`nVars=5336, nPublic=3, domainSize=8192`) matches a fresh `circom 2.1.9` compile exactly. Existing `.zkey` + backend `verification_key.json` are reused unchanged — no trusted-setup ceremony, no backend re-key. The "5286" figure earlier in this doc was approximate; the real artifact is 5313 non-linear constraints.
- **gnark compatibility (gate 2d)** → **YES, zero backend change.** An `ark-circom 0.6` proof (reusing the committed `.zkey`/`.wasm`, generated with the mandatory `CircomReduction` QAP) serialized to snarkjs `proof.json` (G2 limb order `[c0, c1]`) verifies through the backend's exact `circom2gnark`→`gnark.VerifyProof` path against the existing vkey.
- **Multithreaded vs single-threaded WASM** → **start single-threaded** (no COOP/COEP; section 6 skipped). Native proof ≈1.6 s vs snarkjs ~2 s; single-threaded is acceptable. `wasm-bindgen-rayon` + cross-origin isolation is a deferred optimization to validate against the live PWA.
- **`ark-circom` directly vs via mopro** → **direct `ark-circom 0.6` + a hand-rolled `wasm-bindgen` wrapper.** Leaner for a PWA-only target; proven end-to-end in the gate-2 spike. ark-circom 0.6 has native `wasm32` support (wasmer `js-default` runs the witness `.wasm` via the browser's WASM engine).
- **MIT Poseidon source (gate 1.1)** → **author in-repo, regenerate constants.** No off-the-shelf MIT `poseidon.circom` exists (circomlib is GPL; Semaphore/MACI/zk-kit all `include` it). circomlib's constants are generated by the **public Poseidon authors' reference** (`generate_parameters_grain.sage`, IAIK hadeshash) — so an independent MIT generator can reproduce the exact `C/M/P/S` for the required widths (t=2 for `Poseidon(1)`, t=3 for `Poseidon(2)`), verified byte-identical to circomlib and hash-identical to `circomlibjs`. Provenance to be confirmed by counsel.
