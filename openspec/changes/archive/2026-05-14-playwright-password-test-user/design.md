...
## Implementation Deltas (vs. original design)

The cutover-implementation pass exposed three deviations from the original design / tasks. Captured here so the next change in this area doesn't re-discover them; the corresponding tasks.md annotations carry the same notes.

1. **`§3.1` "Playwright MCP first" downgraded to "Playwright Node API headless first"** — D2 originally preferred Playwright MCP as the primary path. Picking MCP would have introduced an `mcp__playwright__*` server runtime dependency on every developer + CI workstation, while the plain `@playwright/test` headless mode was already available in `node_modules` and produced a Playwright-native `storageState.json` shape with no shim layer. The D2 fallback ("Playwright Node API headless") became the actual choice; MCP is no longer required to run the capture. The fallback was always documented as equivalent in shape, so this is a path-priority swap not a spec-behavior change.

2. **`§1.8` verified against `prod`, not `staging`** — there is no `staging` stack in the `liverty-music` Pulumi org today (`pulumi stack ls` returns `dev` + `prod` only). The task wording already allows "any non-dev stack"; `prod` substitutes here. If/when staging is provisioned, re-running the verification against it is the cleanest re-confirmation; the outer `if (env === 'dev')` gating mechanism applies uniformly to any non-dev stack so a different env doesn't change the contract.

3. **Existing-passkey-user dependency is latent — the "Existing Passkey Capture Path Retained" requirement is operationally vacuous against self-hosted Zitadel** — the requirement (`e2e-auth-testing/spec.md`, this change) says `capture-auth-state.ts` is retained so a developer on a display-capable host can drive the OIDC flow "against the passkey-only user". The script IS retained unchanged. But the passkey-only user the scenario implicitly refers to — `pepperoni9+playwright-1@gmail.com` — is a Zitadel-Cloud-era Self-Registration user that was wiped by `self-hosted-zitadel §10`'s `truncate_users_for_zitadel_migration` migration and was never re-provisioned on the self-hosted dev Zitadel. So the script will load Chromium, drive the OIDC flow, and Zitadel will reject the username with "user not found". The script's *retention* is satisfied; its *usefulness* against the active dev Zitadel is not.

   **Resolution path**: a follow-up change `remove-passkey-capture-path` is queued immediately after this change archives. It REMOVES the "Existing Passkey Capture Path Retained" requirement, `git rm`s `capture-auth-state.ts`, and revises `frontend/AGENTS.md` + `.auth/README.md` to a password-only table. This change ships as-is (the spec correctly describes the script's status; only the underlying credential is gone) and the cleanup change closes the loop without scope-creeping the current archive.

   Future-proofing note: if at some point CI-level WebAuthn regression testing becomes a real requirement, that's a new design problem (`webAuthn.addVirtualAuthenticator` + Pulumi-managed device-bound enrollment) — not a fork of the current script's lineage. The cleanup change should not be read as "passkey testing forever abandoned"; it just removes the dead script and outdated requirement.
