...
- **[A brand-new user's marker is lost (blocked storage, cross-tab redirect anomaly) → misses the celebration]** → Accepted; fail-closed per D3. Missing a one-time confetti is strictly less harmful than the current false-positive on every returning sign-in. In practice this is near-unreachable: Passkey is the sole auth method (see the `landing-page` spec), so a genuinely new user cannot complete the `signIn()` / Log In flow — they have no passkey. All new backend accounts (`created === true`) are minted through `signUp()` (`prompt: 'create'`), which always carries the marker. The residual miss is therefore limited to a *partial-signup recovery* (a Zitadel identity exists but the backend user row was never created), where suppressing the celebration is arguably correct.
- **[Direct hit to Zitadel's hosted register page → marker absent but `created === true`]** → The only non-recovery path that can reach the callback with a fresh account and no flow marker: a user landing on `.../ui/v2/login/register` directly, bypassing `signUp()`. Fail-closed (no celebration). Closing this residual is tracked in the parked change `login-register-error-classification` (app-owned registration entry); cross-referenced here so the two residuals are not reasoned about in isolation.
