...
- **WHEN** the user clicks the transparent area above the sheet card (dismiss-zone or scrollWrapper outside sheet-page)
- **THEN** the CE SHALL detect the click by checking that `event.target` is NOT inside `.sheet-page` (using `event.target.closest('.sheet-page') === null`)
- **AND** the CE SHALL initiate a smooth scroll to the dismiss zone (`scrollWrapper.scrollTo({ top: 0, behavior: 'smooth' })`)
- **AND** the sheet SHALL close via the scroll-snap dismiss mechanism (`onScrollEnd`)

#### Scenario: Sheet-page click does not dismiss
- **WHEN** the user clicks anywhere inside the `.sheet-page` element (handle bar, content, buttons)
- **THEN** the CE SHALL NOT initiate dismiss
- **AND** `event.target.closest('.sheet-page')` SHALL return non-null, causing `onBackdropClick` to return early
