...
- [x] 7.3 Frontend released: organizer console cover-upload migrated to the async pipeline (fe v1.62.0), + fixes surfaced during real prod testing — performer-selection double-toggle (v1.62.1), CSP for GCS PUT + media CDN (v1.62.2). Also required backend v1.51.0 (GCSStorer.ReadObject) + cloud-provisioning#471 (organizer-console-api signing env + IAM: internal-bucket write + tokenCreator-on-self).
- [x] 7.4 Prod verified end-to-end (real console upload): organizer uploaded a cover → media-consumer woke from zero → read original → libvips WebP `thumb`+`large` → written to the served bucket → `series_media` cut over → original reclaimed → variant served at `https://media.liverty-music.app/cdn/{org}/{mediaId}/large.webp` (**HTTP 200, image/webp, immutable cache**). EXIF-strip / replace-reclaim / malformed-no-variants are covered by the §4.6 unit tests; a prod spot-check of those edge cases is left to organic use.
