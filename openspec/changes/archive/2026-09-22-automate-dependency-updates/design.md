...
Dependabot cannot read `mise` configuration files or `kustomize` manifests, both of which are live dependency axes here (`cloud-provisioning/.mise.toml`, `specification/.mise/config.toml`, `backend/k8s/`). It also cannot express cross-file grouping, which D4 depends on entirely. Renovate covers eleven of the twelve axes — Go modules, Go `tool` directives, the Go toolchain, npm packages, npm `overrides`, GitHub Actions, Docker base images, mise tools, Pulumi providers, pre-commit hook revisions, and kustomize image tags — supports arbitrary grouping, and supports regex-based custom managers. The twelfth, `buf.lock`, is covered by neither tool — Renovate has no Buf Schema Registry manager or datasource — so it is not a point of comparison between them; it stays manual under either choice (D7). An axis the chosen tool cannot read at all is not simply dropped from consideration: it is recorded as a known gap with the human action that advances it named in its place, the same treatment `buf.lock`, the BSR-generated Go modules (D7) and `pocketsign`'s schema SDK all get below.
...
`allowAutoMerge` is added to `defaultRepositoryArgs` specifically, not to `GitHubRepositoryComponent`: it is a `github.Repository` resource argument, and `GitHubRepositoryComponent` manages branch protection, environments, variables and secrets rather than the repository resource itself. Enabling it does not touch `requiredStatusCheckContexts`, so an automatically merged pull request satisfies exactly the same branch protection conditions — still exactly `['CI Success']` — as a manually merged one, and the merge strategy an automatic merge uses is whichever one the repository already permits (`allowMergeCommit: true` here; see Context).
...
Actions published from within the liverty-music organization are exempt from this pinning — their contents are already under this project's control, so pinning them would add SHA-tracking overhead without a corresponding trust boundary to cross. That is why the 105-reference count above is a count of third-party `uses:` lines only; calls to `.github`'s own reusable workflows, such as `claude-review.yml`, stay on tags.
...
- **A custom manager's regex silently stops matching** after a file is reformatted, leaving one location of a fan-out quietly behind — the exact failure the grouping exists to prevent. → Mostly designed out rather than mitigated: D4 removes the Go and Node duplication instead of synchronising it, so there is nothing left for a regex to miss and the CI assertion that would have guarded one is not needed. One custom manager remains, for Playwright, because its third location is prose in `AGENTS.md`. That one is checked by running the pattern against the real files rather than assumed to match.
...
- **The WebKit gate this change adds is itself unreliable.** Before this change, `playwright.config.mjs` defined `webkit-repro` and `chromium-control` but no CI job invoked either — the `e2e` job ran only `--project=functional --project=pwa` and `smoke` only `--project=smoke --project=onboarding` — and `page-help-sheet-webkit.spec.ts` was excluded from `functional` on the stated grounds that `webkit-repro` covered it, so the spec had zero actual coverage while reading as covered by another project. Once `webkit-repro` was actually executed, it proved intermittently red: 8 passes / 2 failures over 10 local runs, against 3/3 for `chromium-control`. The failure is the defect the spec exists to catch — the page-help bottom sheet dismissing itself shortly after opening, on real WebKit only — so the `IntersectionObserver` "just-opened" guard does not hold reliably. → Not mitigated here, and deliberately not retried away: `retries: 2` will usually mask it, which is precisely the failure mode this change is trying to eliminate elsewhere. It is recorded in `frontend/docs/ci-coverage-gaps.md` as an open product defect. Until it is fixed, WebKit-sensitive automerge rests on a gate that flakes, and the honest reading is that the WebKit gap is *narrowed*, not closed.
