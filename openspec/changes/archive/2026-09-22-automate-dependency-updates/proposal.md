...
Dependency updates across the four liverty-music repositories are entirely manual today — there is no Renovate or Dependabot configuration anywhere. Twelve distinct dependency axes (Go modules, Go `tool` directives, the Go toolchain, npm packages, npm `overrides`, GitHub Actions, Docker base images, mise tools, Pulumi providers, pre-commit hook revisions, `buf.lock` module dependencies, and kustomize image tags) are each bumped by hand, so security patches land only when someone notices them. Eleven of the twelve are automated here. `buf.lock` is not: Renovate ships no Buf Schema Registry manager or datasource, and its entries are registry commit identifiers rather than versions, so it stays a manual `buf dep update` named in the runbook rather than a silently missing axis.
...
- **The Pulumi preview is real, but it is invisible from the repository and its safety is accidental.** An earlier draft of this proposal claimed infrastructure changes were never previewed, on the evidence that `cloud-provisioning`'s GitHub Actions CI runs `make lint-ts` only. That is true of GitHub Actions and false of the project: Pulumi Cloud Deployments has `previewPullRequests: true` on *both* the dev and prod stacks. Nothing in the repository says so — the settings live only in the Pulumi Cloud console, and the state-recovery runbook already cites a `Pulumi.dev.deploy.yaml` that was never committed.

  What the audit should have found is narrower and more specific. The prod stack's trigger `paths` is `src/**` alone, so a pull request editing only `Pulumi.prod.yaml` changes what would be deployed and is previewed by nothing. And the property this change most depends on — that an automated dependency pull request never executes third-party code against production credentials — currently holds only because `paths` happens not to list `package.json`, while `skipInstallDependencies` is `false`. Adding dependency manifests to that filter is an obvious, well-intentioned improvement that would silently open the exposure.
...
- Grouping of version-coupled families into single PRs, reducing an estimated ~100 PRs/month to ~36. Most families (Storybook, Vitest, OpenTelemetry JS, stylelint, Pulumi) are grouped by maintained upstream presets; local rules are written only where no upstream group exists AND the family is genuinely version-coupled. Two qualify: Aurelia (its packages pin each other exactly) and the Go `tool` entries (grouped for noise, explicitly not as a coupling claim). OpenTelemetry Go and Connect-RPC Go were dropped after measuring their declared constraints — both bind by lower bound only, so upgrading one member alone builds fine and upstream's separation of them stands.
...
- **Infrastructure credentials are kept off automated pull requests.** Producing a Pulumi preview means installing the stack's dependencies and executing its program, and the stack resolves production credentials from a shared ESC environment that cannot be narrowed per job. Since Renovate pushes to branches inside the repository, previewing a dependency bump would execute a just-published third-party package with those credentials before anyone read it.

  The control point is therefore *which pull requests reach the preview at all* — and that decision belongs to the Pulumi Cloud deployment settings, not to a GitHub Actions job condition, because that is where the preview is actually triggered. Keeping dependency manifests out of the trigger `paths` stops being an incidental side-effect and becomes a stated, version-controlled control with the reason recorded beside it. Provider updates are reviewed rather than automerged.
...
- `cloud-provisioning/.github/workflows/ci.yml` — new `test` job wired into `ci-success` / `allowed-skips`; `Pulumi*.yaml` added to the paths filter. No preview job: the preview lives in Pulumi Cloud Deployments.
- New: `cloud-provisioning/Pulumi.dev.deploy.yaml`, `cloud-provisioning/Pulumi.prod.deploy.yaml` — the deployment settings brought under version control, so the trigger `paths` that carry the D6 control are reviewable. `Pulumi.prod.yaml` is added to the prod stack's `paths`; dependency manifests are deliberately kept out of both, with the reason recorded inline.
...
- `backend/.github/workflows/{test,lint,stripe-sandbox-e2e}.yml` — the five `go-version` literals replaced by `go-version-file: go.mod`; `backend/.golangci.yml` — its `go` setting deleted so it derives from `go.mod` too. Go's nine locations become two, both natively managed, and no custom manager is written. `backend/go.mod` is unchanged in value; its `toolchain` comment is reworded as a floor record. `backend/Dockerfile` is left alone — its tag is an ordinary container dependency at its own precision.
- `frontend/.nvmrc`, `cloud-provisioning/.nvmrc` (new) — single source for the Node version, read by `setup-node` via `node-version-file`, replacing nine workflow literals.

**Infrastructure**: **No new credential is introduced.** The preview already runs in Pulumi Cloud Deployments, which authenticates to GCP by OIDC (`pulumi-cloud@liverty-music-prod.iam.gserviceaccount.com` via the `external-providers` workload pool) and resolves the Cloudflare, Zitadel and GitHub provider credentials from ESC through `Pulumi.prod.yaml`'s `environment: liverty-music/prod`. An earlier draft proposed adding a `PULUMI_ACCESS_TOKEN` secret so a GitHub Actions job could produce the same preview; that job is not built, because it would duplicate a working mechanism and would trade an OIDC exchange for a stored long-lived token. Since those credentials cannot be narrowed per job, the preview is withheld from automated dependency pull requests rather than run with reduced privileges — enforced in the deployment settings' trigger `paths`.
