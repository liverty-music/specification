...
The catalogue records this as a per-event collection-status column (`active` / `dormant`). A removed event is not left in the live table with a `deleted` status — it moves to a dedicated Removed events section together with its removal reason, so the live table only ever lists what's actually collectible. Dashboards and funnels are built only from `active` events; the primary conversion funnel is re-cut to terminate at `concert.detail.viewed`, the last observable step in the active set, rather than at the now-dormant `entry.zk_proof.verified`.
...
**D5 — Prevent recurrence with a verified-call-site requirement.** The audit only found the phantoms because it read call sites, not the catalogue. An event catalogued as `active` must have at least one verified emission call site in the frontend or backend codebase — a bare name constant or type declaration with no code path that emits it does not qualify as `active`; it is either removed to the Removed events section or, if a real emitter exists but is inactive only because its feature is deferred or externally blocked, catalogued as `dormant` instead. A pull request that adds or changes a catalogued `active` event must confirm a concrete emission call site exists, and an event whose emitter is removed is dropped from the catalogue in the same change — this is the enforcement mechanism, not just an audit afterthought.
