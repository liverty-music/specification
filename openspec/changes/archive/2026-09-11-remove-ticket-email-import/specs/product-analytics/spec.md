...
- **AND** a step SHALL NOT reference a `dormant` event

#### Scenario: Deferred ticketing events are dormant, not deleted
- **WHEN** the catalogue lists an implemented-but-inactive event whose feature is deferred or externally blocked (rather than removed)
...
- **WHEN** an event is removed from active collection (phantom, redundant, double-counting, firehose, wrong-altitude, or a removed capability such as the blockchain ticket flow or the ticket-email import flow)
...
#### Scenario: Ticket-email analytics event is removed
- **WHEN** the ticket-email import capability is removed
- **THEN** `ticket.email.parsed` SHALL be moved from `dormant` to the Removed events section with the reason "ticket-email import capability removed"
- **AND** the event SHALL NOT appear in the live catalogue table
