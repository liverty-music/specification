...
### Decision 5: Surrogate id + best-effort overlap matching for idempotent upsert
Each phase has a surrogate `id` (UUID PK) as its only hard identity and the handle the reminder sent-log references. The discovery job re-runs over the same series, so re-extraction must converge to the same row without collapsing two distinct phases. No business field is perfectly stable: `channel`/`sequence` may default (`UNSPECIFIED`, `0`) for unclassifiable phases, `apply_start_time` is revised as announcements firm up, and even "earliest covered event" drifts because coverage is resolved incrementally (a later run can add an earlier date). So matching's **primary signal is same `series_id` + covered-event overlap** — event IDs are immutable and overlap survives both incremental coverage growth and channel/sequence reclassification, so a reclassified or grown phase converges to its existing row instead of re-keying into a duplicate. `channel`/`sequence` and a frozen `anchor_event_id` are recorded for display and used **only** to keep clearly-different sales (e.g. rounds) separate when coverage overlaps; they are **not** hard-equality match keys (requiring their equality would re-introduce the re-keying trap on the channel/sequence dimension). Per-leg phases have disjoint coverage → no overlap → separate rows. All other fields (`apply_start_time` and the remaining timestamps, `provider_name`, `url`, covered-event set) are last-write-wins. The accepted residual is the opposite, rarer failure: two genuinely distinct phases that share coverage and channel/sequence would be **conflated — one silently overwritten via last-write-wins (data loss)**, not merely duplicated; accepted as a known limitation of fuzzy data, minimized by channel/sequence disambiguation.

Because matching is an application-level best-effort overlap check rather than a hard key, the `sales_phases` table carries no database-level uniqueness beyond the surrogate `id` — no unique constraint over `(series_id, channel, sequence)` or over `anchor_event_id`. `anchor_event_id` is set once at insert as a representative covered event and is never recomputed; it exists for display and disambiguation, not as a match key.
...
**6c — Quiet hours (reminder scan only).** Reminders must not wake users at night. The existing `users.time_zone` column (commented "IANA time zone identifier for scheduling notifications") gives each user's timezone; unset → `Asia/Tokyo`. A reminder due within 22:00–08:00 local is shifted: non-deadline stages (application open, lottery result) defer to 08:00; deadline-relative stages defer to 08:00 if still before `apply_end_time`, otherwise the scan **looks ahead and emits them early on its last run before the window begins** (a scan cannot send at a past time once the window has started), never waking the user and never firing at/after the deadline. Only milestones after a phase becomes known fire. The event-driven announcement (6a) is not subject to quiet hours — it fires from the daily daytime discovery job, not the scan.
