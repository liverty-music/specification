...
- **The inherit path resolves ONLY `:<before>` — no tag fallback.** An earlier draft fell back to `:main` when `:<before>` was missing; that is unsafe. The fallback only ever fires when `:<before>` is unresolvable, which means the parent run *failed* and left a gap — in that state `:main` still points at some earlier commit whose tree may differ from this one (e.g., a partial matrix build-failure on an intervening build-relevant commit), so inheriting it would silently pin non-equivalent bytes (a cross-service prod skew with no error). `HEAD^1` is wrong for the same reason on multi-commit pushes. So the inherit path resolves only `:<before>`, classifies the failure like the release-path resolve (auth → fail fast; transient → bounded retry; `NOT_FOUND` → chain-broken), and on a genuine gap **fails loudly** with a seed-and-re-cut instruction rather than fall back.
