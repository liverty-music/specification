...
- **Zero-recipient fan-out outcomes are logged, not silent.** When new-concert fan-out dispatches zero notifications — no followers, no follower eligible after hype filtering, or no deliverable concerts — it logs the outcome with the reason and the artist identifier, and still completes successfully (an empty outcome is not an error). This closes the other half of the ~2-week blind spot: even once per-delivery failures are observable, an upstream fan-out that quietly skips everyone would otherwise still look like nothing happened.
