...
- [x] 6.1 In `internal/infrastructure/database/rdb/user_repo.go`, extend the `sql.NullString` scan pattern to `country` and `time_zone` (`scanUser` already does it for `preferred_language` per PR #304; the same intermediate-local approach applies). Use the existing `homeID/countryCode/level1/level2` pattern as the reference.
