...
- `SELECT` lists do NOT adopt `COALESCE(..., '')` as the protection mechanism for the columns this change touches. Reasoning: COALESCE masks the distinction between NULL and empty-string at the wire boundary, which we need elsewhere (the proto layer omits the field when the row is NULL). Scanning into `sql.NullString` preserves the distinction while still defending against the crash. `safe_address` is the one exception: it has scanned via `COALESCE(col, '')` since its first commit, predating this change, and that grandfathered pattern is left as-is rather than migrated — it already defends against the crash, so the two patterns coexist per-column rather than being alternatives to pick between on new columns.
