...
### D8. Coverage branch threshold recalibrated for Vitest 4's AST-aware remapping (decided during apply)
Vitest 4's `@vitest/coverage-v8` switched to AST-aware branch remapping (`ast-v8`), which counts many more branches (optional chaining, nullish coalescing, template conditionals, default params) than the pre-4 V8-native mapping. The *same* unit code that measured ~78% branches under Vitest 2 measures ~61% under Vitest 4 — a measurement-basis change, not a real coverage regression, and there is no v4 toggle back to the old mapping. To keep the gate enforced without a large out-of-scope test-authoring effort, the branch threshold is recalibrated **78 → 60** (statements/functions/lines stay at 70). "Unit coverage thresholds are preserved" is honored in intent — enforcement continues at the new metric's floor — with the branch number adjusted to the new methodology (user-approved during apply).
- *Also folded in during apply:* Vitest 4 no longer forwards `--no-experimental-webstorage` execArgv to forked workers, so Node 25's broken Web Storage stub is replaced by an in-`test/setup.ts` polyfill (plus a `matchMedia` stub jsdom lacks); mocked-class `mockImplementation` must be a constructable function expression (not an arrow); `coverage.all:false` restores the imported-files-only scope.
...
- **OQ2 (resolved during apply):** Design-system subset = the four static, deterministic primitives `svg-icon`, `inline-error`, `state-placeholder`, `page-header` (the animated `loading-spinner` and popover/positioned `toast`/`bottom-sheet`/`coach-mark`/`snack-bar` are storied for render/interaction/a11y but excluded from pixel baselines to avoid animation/timing flake). Tolerance = `pixelmatch` `allowedMismatchedPixelRatio: 0.001` (0.1%). Baselines are generated **and** compared inside the pinned official Playwright container (`mcr.microsoft.com/playwright:v1.58.1-noble`), which makes rendering deterministic — so a tight tolerance is both safe (no cross-env AA flake) and sensitive (a loose 2% ratio failed to catch a full icon swap on the mostly-transparent icon canvas; 0.1% catches it). The CI `storybook-test` job MUST run in this same container image for committed baselines to compare validly.
