...
- The contract behind these signatures: `resolveWeekend` follows the day-of-week rule (Monday-Friday resolves to the coming Saturday..Sunday, Saturday resolves to today..tomorrow, Sunday resolves to today..today); `inclusiveDaySpan` counts both endpoints (equal dates span `1`); `parseDateInput` rejects empty/malformed strings and out-of-domain month/day components by returning the "no value" sentinel rather than throwing.
