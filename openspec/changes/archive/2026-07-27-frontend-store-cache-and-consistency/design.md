...
- **Every cached resource has a defined invalidating write, so no write leaves a stale cache.** A successful `follow`/`unfollow` and a successful `setHype` both invalidate `listByFollower` (hype is rendered from that same data). A ticket mint invalidates `listTickets`. `listWithProximity` needs no write-side invalidation — it is keyed by the guest artist-id set, so a changed follow set produces a new key rather than staling the old one. `listTop` likewise needs none — it has no user-write inputs and relies on `staleTime` plus resume revalidation for freshness.
