...
Extract each route's fetch body into a dedicated method that returns a `Promise` (`loadArtists()`, the existing `loadData()`, `loadInitialBubbles()`), then call it with `void` from `loading()`. **Each method owns its request `AbortController`** — abort-first then create, then run the fetch with `this.abortController.signal`, mirroring the existing `loadData()` exactly. The synchronous prelude that stays in the `loading()` body is `isLoading = true`, URL filter restoration, guest hydrate, and banner flags; it does NOT create the `AbortController`. The controller is instead created synchronously at the head of the fetch routine (before its first await, still within `loading()`'s synchronous call stack), so it exists before first paint without `loading()` and the routine contending for ownership.

- **Single controller owner (avoids the abort-self bug)**: if `loading()` created the controller AND the routine also abort-and-created, a normal navigation would pass an already-aborted signal into the fetch (the routine aborts the controller `loading()` just made) → immediate `AbortError` → the route never loads. Giving the routine sole ownership, as `loadData()` already does, removes the contradiction.
- **Why over moving the fetch into `attached()`**: keeping the prelude colocated in `loading()` sets `isLoading`/filters before the first render. `attached()` runs after first paint, so prelude state placed there would render one frame late. The `void`-in-`loading()` form also matches the precedent already in the Dashboard `needsRegion` branch.
