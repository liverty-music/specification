...
> **0.1 (Stripe live-mode Connect account + KYC/審査) and 0.2 (収納代行 counsel opinion) are SPLIT OUT to a live-money-launch follow-up** and are OUT OF SCOPE for this change. This change ships + prod-verifies the **test-mode-functional ④** (no real charges); 0.1/0.2 gate only the real-money launch. Tracked in specification issues **#920** (0.1 live Connect/KYC — recommended Accounts-v2 config + the Connect-param backend follow-up on the manual-capture PI, seam prepared in be #434) and **#902** (0.2 収納代行 counsel; parent research track #778). They do not block archive of this change.
...
- [x] 7.2 End-to-end verify: configure phase → apply (authorize, no capture) → close → draw (no oversell, groups intact, ordered loser waitlist; **winners captured / losers released**) → ⑤ issuance handoff — **backend pipeline VERIFIED against real Postgres + real Stripe test API 2026-09-02** (backend PR #431, `internal/infrastructure/database/rdb/lottery_pipeline_integration_test.go`): capacity-2 phase, 3 fans each authorize a real hold + apply → draw → asserted **2 winners captured (PI `succeeded`) / 1 loser released (PI `canceled`), no oversell, loser draw_sequence recorded**. Adapter round-trip + idempotency also verified (backend PR #430). Both tests are opt-in (`STRIPE_INTEGRATION_TEST=1` + `sk_test_`; make check/CI skip). Frontend `StripeService` unit-covered 100% (fe PR #583). The remaining **browser-3DS + OIDC E2E** is NOT run against dev (dev Zitadel intentionally stopped) — it is instead done **in prod on Stripe test-mode** (§8.4 below), since prod Zitadel is up and no real charges occur. ⑤ issuance handoff is ⑤'s scope (0/29, not built). **DONE 2026-09-04:** the browser E2E remainder was completed in prod per §8.4 — real fan apply (`4242…`) → hold → draw → **winner captured** (PI `succeeded`, DB Won). The prod run also exposed + fixed the draw-sweeper Stripe-gate bug (backend #440 / v1.53.2); see §8.4.
- [x] 7.3 Sync delta specs to main specs and archive the change — **DONE 2026-09-04**: §8.4 passed (prod test-mode E2E, winner captured). 0.1 (live Connect/KYC → #920) and 0.2 (#902) split out to the live-money-launch follow-up (see §0 note); they do NOT block archive of the test-mode-functional ④. Delta specs synced to main specs and the change archived.
...
- [x] 8.4 **Prod E2E verify (browser, test-mode, no charges):** OIDC login (prod Zitadel is up) → organizer configures a lottery phase → fan applies with a 3DS **test card** (`4242…` success / `4000 0027 6000 3184` 3DS) → close → draw → winners captured (test) / losers released → my-application result screen. Satisfies the ④ end-to-end goal in prod. (Amex test card rejected at entry.) **VERIFIED 2026-09-04:** real browser — organizer `Configure lottery` (v1.65.0) → phase (¥50/cap1/max1) → fan applied with `4242…` → PI `pi_3UBv9C…` = `requires_capture` ¥50 (hold, no capture) → close (`close_at=now()`) → **fan-api draw sweeper** ran → **winner captured: DB `ticket_applications.state=2` (Won), PI `succeeded` amount_received=50, winner_count=1**. ⚠️ Verification found + fixed a **prod bug**: the draw sweeper started in EVERY server-binary workload (fan-api + admin/organizer consoles) but only fan-api has `STRIPE_SECRET_KEY`; a Stripe-less admin pod winning the idempotent draw race demoted every winner to a loser (capture unavailable). Fixed by gating `startLotteryDrawSweeper` on `cfg.Stripe.SecretKey != ""` → **backend #440 / Release v1.53.2** (post-deploy logs: fan-api `sweeper started`; admin/organizer `sweeper disabled`). Loser-release path stays covered by 7.2's #431 integration test (+ the pre-fix stranded ¥50 hold released via Stripe API).
