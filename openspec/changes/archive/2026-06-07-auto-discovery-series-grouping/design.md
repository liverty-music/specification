...
- **Cross-source `start_at` disagreement** → split. Because `start_at` is in the key (to distinguish 昼夜2公演), the same physical show splits into two events/series if two sources publish *different* concrete start times for it — e.g. one official site lists the door time and another the performance time, or a schedule changes between scrapes. This is the symmetric cost of Decision 2: the same mechanism that keeps genuine matinee/evening shows distinct also splits a single show on time-disagreement. Narrow (one side is often NULL, and the pipeline keeps `open_time` separate from `start_time`); accepted, logged, monitored. *Optional hardening:* a small start-time tolerance window, or falling back to `(venue, date)` when performer sets overlap — both risk merging real 昼夜, so deferred.
...
- **Tour stops pre-split across two series are not reconciled.** If two stops of one tour were independently persisted earlier under different series, a later run that groups them adopts only the first-matched stop's series for the new rows; each already-persisted stop's row keeps its own `series_id` (the UPSERT never rewrites `series_id` on conflict). The tour stays split. Accepted; the FK-adoption model converges newly-discovered stops but does not actively merge pre-existing divergent series (same family as the 追加公演 residual; anchor reconciliation deferred).
- **Event-key precision / timezone mismatch (latent).** The application encodes the physical start key as `entity.StartKey` = UTC `RFC3339` at **second** precision, while the DB constraint and the `event_performers` JOIN compare raw `timestamptz` at microsecond precision. They agree today only because the discovery pipeline emits whole-minute times. A future writer landing a sub-second `start_at` could make the app-layer dedup/adopt decision disagree with the DB constraint. *Hardening:* normalize `start_at` to whole-second UTC once at ingestion so stored value == `StartKey` input == DB key. Deferred to Open Questions.
- **Migration event-collapse cascade.** The dedup step in the migration repoints only `event_performers` to the survivor; a loser event's other FK children (`concerts`, `event_sales_phases`, `sales_phases.anchor_event_id`, ticket/journey tables) cascade-delete. Harmless pre-launch (those tables are empty), but it is a hand-picked single-table merge, not a general one. Run only pre-launch, or add an emptiness guard / generalize the repoint before reusing the pattern post-launch.
...
- **DB migration (Atlas), backend repo:**
  ...
  3. Backfill `series.id` to pure `UUIDv7`, then tighten the CHECK:
     - Re-key any existing v5 `series.id` to a fresh `UUIDv7`, cascading the new id to every FK referencer (`events.series_id`, `sales_phases.series_id`, and any other `series` reference) in one transaction. Pre-launch data volume is negligible (dev intentionally stopped; prod pre-launch on v1.x), so this is cheap and a no-op when no v5 rows exist.
     - `DROP` `chk_series_id_uuid_v5_or_v7`; `ADD` `chk_series_id_uuidv7 CHECK (...)`. Because the backfill leaves only v7 ids, the new CHECK validates immediately — no `NOT VALID` escape hatch is needed (PostgreSQL validates a new CHECK against all existing rows at `ADD CONSTRAINT` time, which would fail if any v5 row remained).
     This fully removes the v5 carve-out per Decision 4. The re-key is a hand-authored data step (Atlas `diff` produces only schema DDL), captured in tasks.md task 3.
...
- **Canonical start-time storage:** whether to normalize `start_at` to whole-second UTC at ingestion (in the searcher's coercion) so the stored value, `entity.StartKey`, and the DB constraint share one precision — removing the latent precision/timezone mismatch above. Also: route the Step-2 searcher's own `(date, venue, start_time)` dedup through `entity.StartKey` so "same physical show" has a single encoding across all layers.
