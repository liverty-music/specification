...
- **Risk: the mechanical `HypeLevel` → `entity.hype` mapping breaks for entities like `EventDateGroup` or compound names** → Mitigation: documented in the spec with explicit examples; the lint script maintains a curated entity-name list and treats an `entity.*` stem outside that list as a lint failure (reported with the offending key path) rather than silently accepting it — the stem is added to the curated list as part of the same change that introduces the entity's label. Worst case: future entities document an explicit override.
