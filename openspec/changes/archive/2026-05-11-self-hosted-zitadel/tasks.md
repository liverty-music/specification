...
- [x] 17.1 `/opsx:verify self-hosted-zitadel` run on 2026-05-11 — **all 4 §13 incident-driven divergences are already captured in the delta specs**: (a) auto-verify-email removal landed as REMOVED Requirement in `specs/identity-management/spec.md` + `specs/zitadel-action-webhook/spec.md`; (b) webhook JWT `iss`/`aud` enforcement removal landed as updated rationale in `specs/authentication/spec.md` + `specs/zitadel-action-webhook/spec.md`; (c) SMTP activation gap landed as ADDED Requirement *"SMTP Configuration Must Be Activated After Creation"* in `specs/identity-management/spec.md`; (d) LoginClient PAT requirement landed as ADDED Requirement *"Dedicated Service User for Login V2 UI"* in `specs/identity-management/spec.md`. Live implementation cross-checked: backend `pre_access_token_handler.go` (commit `1143498`), `auto_verify_email_handler.go` cleaned via PR #290 (`bb4a3e8`), iss/aud checks dropped via PRs #288/#289, `ZitadelSmtpActivation` Dynamic Resource exists at `cloud-provisioning/src/zitadel/dynamic/smtp-activation.ts`, `LoginClientComponent` at `cloud-provisioning/src/zitadel/components/login-client.ts`. Backend → Zitadel auth path live-tested same day: `backend-app` machine key from GSM successfully exchanges JWT-bearer assertion at `/oauth/v2/token` and lists 1 user via `/v2/users`. **No critical issues.** 13 outstanding tasks (§14 / §17.2 / §18.5 / §18.7 / §18.8 / §18.9) all have explicit destinations (frontend#345 or spawned changes); none are spec-implementation gaps.
- [x] 17.2 `/opsx:archive self-hosted-zitadel` performed in this PR — moves the change to `openspec/changes/archive/2026-05-11-self-hosted-zitadel/` and folds the 4 delta capability specs into `openspec/specs/`.
...
- [x] 18.6.1 During the cutover incident chain (multiple `_activate`, MachineKey replace, Action delete, email change API calls within ~30 minutes against a 3.5-day-old pod), the Zitadel API container hung on `GetAuthRequest` for 30+ seconds and returned `code: internal`. Resolved by `kubectl rollout restart deploy/zitadel`. Likely cause: in-memory projection updater stuck on a write lock, or Cloud SQL connection-pool exhaustion from accumulated leaked connections in async notification-worker retries. *Resolved 2026-05-03 by `zitadel-observability` — PR-1 (#220, #222, #223) ships the detection signals (latency p99 alert + JWT error rate alert + Cloud SQL connection-pool dashboard) and PR-2 (#221) ships the band-aid weekly restart CronJob + operator runbook covering the forensic capture + mitigation flow.*
- [x] 18.6.2 Add a Cloud Monitoring alert on Zitadel API request `duration_p99 > 10s` for `OIDCService/*` paths so the next occurrence triggers a page rather than being discovered by a user. *Resolved 2026-05-03 by `zitadel-observability` — PR-1b #223 provisions the `Zitadel OIDCService p99 latency` alert (filter `metric.type="workload.googleapis.com/rpc.server.duration" AND metric.labels.rpc_service=monitoring.regex.full_match(".*OIDCService.*")`, ALIGN_PERCENTILE_99, threshold 10000ms over 60s).*
- [x] 18.6.3 Consider periodic Pod recreation (e.g., a CronJob that does `kubectl rollout restart deploy/zitadel` weekly) for dev. For prod, target the underlying issue rather than recycling. *Resolved 2026-05-03 by `zitadel-observability` — PR-2 #221 ships `cronjob-restart-zitadel` in the dev overlay (Sunday 03:00 UTC, `liverty-music.app/temporary` annotation for cleanup discovery, minimum-privilege RBAC scoped to `deployments/zitadel` only). Prod is intentionally NOT covered — once root cause is found, the CronJob is removed, not promoted.*

- **Tracked in**: openspec change `zitadel-observability` (archived 2026-05-03 — see `openspec/changes/archive/2026-05-03-zitadel-observability/`).
