...
### D7. Bootstrap admin machine key via `ZITADEL_FIRSTINSTANCE_*` and an in-pod sidecar container

The first time Zitadel starts on an empty database, `start-from-init` reads `ZITADEL_FIRSTINSTANCE_*` env vars to create the initial instance. Setting `ZITADEL_FIRSTINSTANCE_MACHINEKEYPATH=/var/zitadel/bootstrap/admin-sa.json`, `ZITADEL_FIRSTINSTANCE_ORG_MACHINE_MACHINE_USERNAME=pulumi-admin`, `ZITADEL_FIRSTINSTANCE_ORG_MACHINE_MACHINEKEY_TYPE=1` (JSON key) causes the Zitadel container to write a JWT-profile JSON to an `emptyDir` volume shared with a `bootstrap-uploader` **sidecar container co-located in the same Zitadel API Pod**. The sidecar uses Workload Identity to upload the JSON to GSM secret `zitadel-admin-sa-key` and then idles (`tail -f /dev/null`) so the Pod stays ready. On the next Pulumi stack apply, `@pulumiverse/zitadel`'s Provider reads the `jwtProfileJson` directly from GSM via the GCP client SDK within the Pulumi program and proceeds to configure Project/App/LoginPolicy/SMTP/Actions v2 against the running instance.

A separate Kubernetes `Job` was rejected because `emptyDir` volumes are scoped to a single Pod's lifetime — two distinct Pods cannot share one. The original "Job + sync-wave" design would have left the Job pod waiting forever for a key file written into a different Pod's volume. A `PersistentVolumeClaim` to bridge the two Pods would work but adds storage configuration for a one-time bootstrap. The sidecar-container approach reuses the in-Pod `emptyDir` natively, requires no sync-wave ordering, is effectively idempotent on Pod restarts (Zitadel only writes `MACHINEKEYPATH` on the first boot against an empty database; on subsequent restarts the `emptyDir` is empty and the sidecar has nothing to upload), and leaves no lingering Job resource cluttering the namespace.
...
3. **K8s manifests**: ArgoCD syncs `k8s/namespaces/zitadel/overlays/dev` — this brings up the Deployments. The Zitadel API container runs `start-from-init` with `FIRSTINSTANCE_*` env vars and writes the admin SA key to an `emptyDir` shared with the `bootstrap-uploader` sidecar container in the same Pod. The sidecar uploads the file to GSM and unlinks the local copy, then idles. No ArgoCD sync-wave ordering is required; the sidecar's volumeMount is naturally satisfied because both containers run in the same Pod.
