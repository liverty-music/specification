...
- [ ] 2.1 Create Organizer connected account; onboarding link; **KYC/KYB status gate**
      ...
      [PARTIAL: code + port + status-gate + onboarding-link done; the live Accounts v2
      account CREATE is still `TODO: wire when SDK/platform` and gated on 0.1 — leave
      unchecked until the connected account can actually be created]
...
- [ ] 4.2 Reserve past the dispute window; chargeback after payout draws on reserve /
      ...
      [PARTIAL: chargeback→reversal path done (dispute webhook → RefundOrder(DISPUTE) →
      transfer_reversal); the reserve / negative-balance-responsibility half is Stripe
      account config (`losses_collector = application`) owned by cloud 5.1 + gated on 0.1 —
      leave unchecked until that lands]
