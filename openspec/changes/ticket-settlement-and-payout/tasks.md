...
- [x] 2.1 Create Organizer connected account; onboarding link; **KYC/KYB status gate**
      ...
      [code+port done; live Accounts v2 create is `TODO: wire when SDK/platform` gated on 0.1]
...
- [x] 4.2 Reserve past the dispute window; chargeback after payout draws on reserve /
      negative-balance responsibility [dispute webhook → RefundOrder(DISPUTE) → reversal; reserve = losses_collector cloud 5.1]
