...
- [ ] 4.5b Full invite-onboarding E2E on the SHIPPED backend flow (design D5 "open validation"): create a fresh Organizer via admin → operator receives the `CreateInviteCode`-sent console-pointed invite email → "Accept invite" → console → OIDC → `/verify` (code) → passkey → `/welcome`. NOT yet run against backend v1.38.0 with a genuinely-uninitialized operator (4.5a verified only the frontend `login_hint`/redirect half; org-test-7 completed the pre-v1.38.0 invite shape). This is the gate before archive.
