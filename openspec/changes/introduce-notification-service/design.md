...
- **[Risk] Added write on the delivery path** (was fire-and-forget) → a DB hiccup could now affect sends. Mitigation: a single indexed insert + one update; on insert failure, fall back to best-effort send + log (don't drop the notification); revisit async-record if latency shows up.
