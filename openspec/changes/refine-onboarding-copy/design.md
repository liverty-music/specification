...
- **[Trade-off]** The `eventDetail.journeyStatus.*` JA translations are best-effort interpretations of the enum semantics (`tracking` = 追跡中, `applied` = 申込済み, etc.). Product may want to refine these — they are reviewable copy, not technical constants.
