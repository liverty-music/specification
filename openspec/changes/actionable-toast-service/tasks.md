## 1. Extend ToastNotification Service

- [ ] 1.1 Define `ToastOptions` interface (`duration?: number`, `action?: { label: string, callback: () => void }`) and `ToastHandle` interface (`dismiss(): void`)
- [ ] 1.2 Change `show()` signature from `(message, severity?, durationMs?)` to `(message, severity?, options?): ToastHandle`
- [ ] 1.3 Update `ToastItem` to include optional `action` field
...
- [ ] 3.1 Update all existing `toastService.show(msg, severity, durationMs)` calls to use `{ duration: durationMs }` options object (search all `.ts` files)
...
- [ ] 4.3 Rewrite `unfollowArtist()` to use `toastService.show()` with action
- [ ] 4.4 Update `undo()` to call `undoHandle.dismiss()`
- [ ] 4.5 Update `commitPendingUnfollow()` to work with new toast lifecycle (callback on timeout)
