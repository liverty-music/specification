...
- **R2: Accidental enablement on dev AR** → Would break ArgoCD Image Updater (dev's `:latest` rewrite). Mitigation: Pulumi conditional is `environment !== 'dev'` (fail-secure negative form, per D2); only the dev stack gets `dockerConfig = undefined`; every other environment (prod, future staging, prod-replica) automatically inherits `immutableTags: true` without an opt-in change. See D2 for full rationale. The PR review SHALL eyeball that the conditional uses the negative form, not `=== 'prod'`.
