...
**Reason**: The merch-discovery CronJob is deleted with the retired merch-discovery capability, so it no longer emits logs to alert on.
**Migration**: None. The `merch-discovery`-specific Alert Policy is removed from `monitoring.ts`.
