...
#### Scenario: App shell uses CSS Grid layout
...
- **AND** the `<main>` element SHALL occupy the `1fr` row with `overflow-y: auto` for scrollable content
...
- **WHEN** any route component renders inside the `<main>` element
- **THEN** the route component SHALL NOT apply height constraints (e.g., `h-screen`) or bottom padding (e.g., `pb-14`) to account for the navigation bar
- **AND** the CSS Grid layout SHALL ensure the route content fills the available space within the `1fr` row
