...
- [ ] 2.1 Find the v2 (`2.14.x-…`) BSR build strings for `@buf/liverty-music_schema.bufbuild_es` and `@buf/liverty-music_schema.connectrpc_es` at the current schema commit (`npm view … versions --json`)
- [ ] 2.2 Update `package.json`: `@buf/*_es` → v2 pins, `@bufbuild/protobuf` → `^2`, `@connectrpc/connect` → `^2`, `@connectrpc/connect-web` → `^2`; regenerate `package-lock.json`
...
- [ ] 3.5 Replace all remaining `createPromiseClient` usages with `createClient` (4 files)
