...
- [ ] 4.5 Smoke test from `zitadel-web` Pod (read-only, no PAT exposure): `wget -O- http://zitadel-api.zitadel.svc.cluster.local/.well-known/openid-configuration` returns 200 with a body whose `issuer` matches the env's public URL.
...
- [ ] 5.1 Update [`cloud-provisioning/k8s/namespaces/zitadel/base/deployment-web.yaml`](cloud-provisioning/k8s/namespaces/zitadel/base/deployment-web.yaml) so `ZITADEL_API_URL` reads `http://zitadel-api.zitadel.svc.cluster.local`. Rewrite the surrounding comment block (lines 60–82) to record the hairpin incident root cause, the new cluster-internal path, and the link to this change. *(Implementation already drafted and proven via `kubectl kustomize` in the Phase 1 worktree; the diff was reverted from Phase 1 to keep the URL flip in Phase 2 per gate decision.)*
