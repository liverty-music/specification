...
- [ ] 3.3 Add a `pulumi-preview` job running `pulumi preview --diff` against the prod stack, with its matching `pulumi-preview-skip` job, wired into `ci-success` / `allowed-skips`. The job MUST trigger on `pull_request`, never `pull_request_target`. Confirm the workflow contains no `pulumi up` path.
- [ ] 3.3b Skip the preview job on automated dependency pull requests (design D6). Running it would `npm ci` a just-published package and execute it with the ESC-resolved production credentials before anyone has read it, and Renovate pushes to branches inside the repository so the fork protections do not apply. The skip job must still satisfy `ci-success` so these PRs are not left permanently pending.
...
- [ ] 3.5 Verify on a member-authored no-op PR that the preview reports zero changes, and on a deliberately resource-altering PR that it reports the change. Separately verify that a Renovate PR in `cloud-provisioning` reaches `CI Success` WITHOUT the preview job having run.
