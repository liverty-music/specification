...
- **`github.event.before` is `0000…` on branch-creation / force-push / first run.** → Fall back to resolving the parent via `git rev-parse HEAD^1` against the dev `:<sha>` tag, or via the `:main` tag's current digest; if no parent image is resolvable at all, fail with an explicit seed instruction (touch a build-relevant file). This is a rare, recoverable edge, not steady state.
