...
- [ ] 0.1 **Onboard Pocket Sign (PocketSign Verify)**: 加盟契約 (ride their 認定PF status — no own 主務大臣認定), register on PocketSign Platform, ¥300k init + per-verify fees, use the free sandbox for PoC (~2-3mo Verify integration lead time) <!-- MVP DECISION 2026-09-03: 加盟契約 DEFERRED to POST-MVP. The MVP runs the FREE mock/sandbox (verify.mock.p8n.app) in ALL envs INCLUDING PROD (see §7 deployment tasks + design "MVP scope & corrections"); the ¥300k contract + prod tenant (verify.p8n.app) are a single config flip made only after the MVP is proven. This task stays open but is NO LONGER an MVP blocker. -->
...
- [x] 5.2 When an event requires verification, prompt UNVERIFIED fans to verify (JPKI) before applying; **clearly inform** them of the requirement <!-- MVP = JPKI-only, no fallback option to offer here. DONE 2026-09-03 (frontend, not yet PRed): LotteryApplyRoute gains optional `verificationRequired` route param; a requiring phase loads GetMyVerificationStatus on entry and parks UNVERIFIED fans on a new `verify-required` step (informative panel + CTA routing to /settings verify entry point). Fails OPEN on status-load error (backend #435 re-validates at Apply → FAILED_PRECONDITION). +5 gate tests (21 green), make lint 0 errors. -->
...
- [ ] 7.1 Cross-repo release order: spec → BSR → backend → frontend; wire ④/⑤ consumers <!-- spec v0.59.0/v0.60.0 released + BSR gen done; backend #427/#432/#435/#436 + frontend #578/#581/#582/#584 all MERGED TO MAIN. What remains = the prod DEPLOYMENT of that code (7.2–7.6), which the original tasks never captured. -->

<!-- DEPLOYMENT for prod sandbox E2E (added 2026-09-03). Verified GAP: prod runs NONE of this — backend prod pin is v1.51.0 which predates ④ #426 + all eKYC PRs, and cloud-provisioning has ZERO POCKET_SIGN_* config. Per the MVP decision (design "MVP scope & corrections"), prod points at the MOCK sandbox host verify.mock.p8n.app, so no 加盟契約 (0.1) is needed to run this. -->
- [ ] 7.2 **Backend prod Release + pin**: cut a backend Release vX.Y.Z from main (must include ④ #426 lottery + eKYC #427/#432/#435/#436) → bump-prod-pin rewrites `backend/api` `newTag` in `cloud-provisioning/k8s/namespaces/backend/overlays/prod/kustomization.yaml`. Confirm the rendered prod image contains `identity_verification_handler.go` + `pocketsign.StampClient` + the lottery Apply gate.
- [ ] 7.3 **cloud-provisioning: wire `POCKET_SIGN_*` for prod fan-api (mock/sandbox)**:
  - GSM secret `pocket-sign-token` in project `liverty-music-prod` holding the **sandbox `sk_mock_…` token** (declared in Pulumi `src/`, value injected out-of-band; sandbox creds are low-risk but still a Secret).
  - ESO: add `POCKET_SIGN_TOKEN → pocket-sign-token` to `k8s/namespaces/backend/base/fan-api/external-secret.yaml` (`fan-api-backend-secrets`).
  - `k8s/namespaces/backend/overlays/prod/fan-api/configmap.env` (+ dev overlay): `POCKET_SIGN_BASE_URL=https://verify.mock.p8n.app`, `POCKET_SIGN_TENANT_ID=<sandbox tenant>`, `POCKET_SIGN_CALLBACK_URL=https://liverty-music.app/verify/callback` (fan-web apex). NB `PocketSignConfig.Validate` requires all-or-none + https + `*.p8n.app` host (mock passes).
- [ ] 7.4 **DB migrations reach prod**: confirm `verified_identities` (#427), `lottery_sales_phases.verification_requirement` (#435), and the ④ lottery tables (#426) migration files are registered in the prod Atlas overlay (`k8s/atlas/**` + kustomization `configMapGenerator.files`) and applied by the `backend-migrations` AtlasMigration Application before fan-api rolls.
- [ ] 7.5 **Frontend prod pin**: ensure the frontend prod pin (bump-prod-pin `component:frontend`) includes #581 (`/verify/callback` route) + #584 (apply verify-gate); confirm `https://liverty-music.app/verify/callback` is reachable (auth-required route) on the deployed prod bundle.
