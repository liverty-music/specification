...
- [ ] 2.1 In `frontend/src/routes/dashboard/dashboard-route.ts`, relocate the re-entry **cache fast-path** so the render-driving `dateGroups` assignment does NOT run in the pre-render `loading()` path. Trigger the load from `attached()` and wrap the cached-timetable assignment in `queueAsyncTask(() => { … })` (from `aurelia`) so it lands on the frame AFTER the shell's first paint. `attached()` alone is insufficient (it is pre-paint) — the Task Queue hop is the load-bearing part.
