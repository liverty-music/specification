<!-- spec: bubble-replenishment | target: components/infrastructure/fan/web/route/discovery | flags: CLASSNAME | new_name: Canvas replenishes bubbles when similar artists are exhausted -->
...
- **THEN** the BubbleManager SHALL call `loadReplacementBubbles()` to fetch fresh artists from the top-artist pool
- **AND** any unseen artists SHALL be spawned as new bubbles near the absorption point via BubbleManager
...
- **THEN** the BubbleManager SHALL spawn only the 5 unseen artists as new bubbles
