...
### Requirement: Configurable default RPC call timeout

Every Connect-RPC call issued through the frontend's shared main-thread transport SHALL be bounded by a default client-side deadline so that a hung or unresponsive backend fails within a known window instead of leaving the request pending indefinitely. The default deadline SHALL be applied uniformly to all frontend applications (fan-web, admin, organizer) that construct RPC clients from the shared transport, and SHALL NOT require per-call or per-method configuration at the call site. This requirement scopes only to RPC calls made through the shared main-thread transport; RPC calls originating in the Service Worker (which has no access to the main-thread dependency-injected configuration) are out of scope for this requirement, as the Service Worker's own network calls are bounded separately. The default RPC timeout value SHALL be sourced from the runtime configuration (`/config.json`) so that it can be tuned per environment without rebuilding the SPA bundle. When the runtime configuration does not specify a value, or specifies an invalid value (not a positive number), the system SHALL fall back to a built-in default of 10 seconds, and bootstrap SHALL succeed without error.
